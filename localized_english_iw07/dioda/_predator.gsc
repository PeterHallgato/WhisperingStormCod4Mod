...
predator() {
	...
	//self.isGod = true;
	self dioda\_utils::setHealth(9999);
	...
	dioda\_utils::TriggerEarthquake(2,1.6,self.origin,450);
	...
	for(time = 0; time < 540; time++) {
		...
		if(speed == 70) {
			...
			dioda\_utils::TriggerEarthquake(.15,1.6,forward,450);
		}
		else
			speedlimit++;
		fov-=(.015/70*speed);	
		...
		if(collision["surfacetype"] != "default" && collision["fraction"] < 1 || (z-100) > self.origin[2]) {
			...
			dioda\_utils::TriggerEarthquake(3,1.6,expPos,450);
		//	radiusDamage(expPos,400,300,120,self,"MOD_PROJECTILE_SPLASH","artillery_mp");
			players = dioda\_utils::getAllPlayers();
			for(i=0;i<players.size;i++) {
				if(players[i] dioda\_utils::isRealyAlive() && players[i] != self && (players[i].pers["team"] != self.pers["team"] || !level.teambased) && distance(players[i].origin,self.origin) < 300) {
...
reaperExitFunctons(player) {
	if(isDefined(player)) {
		...
		if ( level.hardcoreMode )
			player dioda\_utils::setHealth(30);
		else if ( level.oldschool )
			player dioda\_utils::setHealth(200);
		else
			player dioda\_utils::setHealth(100);
		player.visible = true;
		player show();
		player.reaper = false;
