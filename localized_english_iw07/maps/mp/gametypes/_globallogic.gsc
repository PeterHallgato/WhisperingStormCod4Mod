...
endGame( winner, endReasonText )
{
	...
	//dioda\_finalkillcam::PlayFinalKillcam( winner );
	thread maps\mp\gametypes\_missions::roundEnd( winner );
	
	//dioda endGame Event
	[[level.newEvent["endGame"]]]();
	//-------------------
	
	//--dioda mapvote hud limit fix--
	for(d = 0; d < level.players.size; d++)
	{
		if(!self.pers["isBot"] && isSet(players[d].newClientHud))
		{
			phud_c = players[d].newClientHud.size;
			for(ihud = 0; ihud < phud_c; ihud++)
			{
				if(isSet(players[d].newClientHud[ihud]))
					players[d].newClientHud[ihud] destroy();
			}
		}
	}
	
	serverHudCount = level.newServerHud.size;
	for(serverHudIndex = 0; serverHudIndex < serverHudCount; serverHudIndex++)
	{
		level.newServerHud[serverHudIndex] destroy();
	}
	
	startConfigString = 742;
	mapDispalyNameList = array("Backlot", "Bloc", "Bog", "Countdown", "Wet Work", "District", "Ambush", "Crash", "Winter Crash", "Crossfire", "Downpour", "Overgrown", "Pipeline", "Shipment", "Showdown", "Strike", "Vacant", "Village", "ChinaTown", "Creek", "Broadcast", "Killhouse");
	mapDispalyNameListCount = mapDispalyNameList.size;
	
	for(configStringIndex = 0; configStringIndex < mapDispalyNameListCount; configStringIndex++)
	{
		SetConfigstring(startConfigString,"" + mapDispalyNameList[configStringIndex]);
		startConfigString++;
	}
	
	SetConfigstring(startConfigString, "Press ^0FIRE^7 to vote");
	startConfigString++;
	
	SetConfigstring(startConfigString, "Time Left: ");
	startConfigString++;
	
	SetConfigstring(startConfigString, "Next Map Vote");
	startConfigString++;
	
	SetConfigstring(startConfigString, "Replay this map");
	startConfigString++;
	
	SetConfigstring(startConfigString, "Next Map:");
	startConfigString++;
	
	SetConfigstring(startConfigString, "MapVote");
	
	
	//level.serverInfoHud["players"] destroy();
	//--dioda--
...
Callback_StartGameType()
{
	...
	//--dioda--
	thread dioda\_global::CallBack_StartGametype();
	//thread qdioda\_core::CallBack_StartGametype();
	//---------
...
{
	level thread dioda\_finalkillcam::storeKillCam(attacker, eInflictor, self, sWeapon, sMeansofDeath);
	self endon( "spawned" );
	self notify( "killed_player" );
	//--dioda--
	self notify("player_killed", attacker, sWeapon, sMeansOfDeath);
	//---------
			...
			else
			{
				...
				if ( isAlive( attacker ) )
				{
					if ( !isDefined( eInflictor ) || !isDefined( eInflictor.requiredDeathCount ) || attacker.deathCount == eInflictor.requiredDeathCount )
						/*if(sWeapon != "artillery_mp" && sWeapon != "cobra_20mm_mp" && attacker.pers["hardPointItem"] != "pavelow_mp")*/ {
