...
init()
{
	...
	priority++;
	level.hardpointItems["pavelow_mp"] = priority;
	priority++;
	//preadtor end---
	...
	//predator---
	level.hardpointHints["predator_mp"] = "Press 6 for PREDATOR";
	level.hardpointHints["pavelow_mp"] = "Press 6 for Pavelow";
	//preadtor end---
	...
	level.hardpointHints["pavelow_mp_not_available"] = "Pavelow not available";
	//preadtor end---
	...
	level.hardpointInforms["pavelow_mp"] = "mp_killstreak_heli";
	//preadtor end---
...
giveHardpointItemForStreak()
{
	...
	if(!isset(getDvarInt("d_killstreak_offset")))
		offset = getDvarInt("d_killstreak_offset");
	else
		offset = 0;
	...
	if ( streak == (3 + offset) )
		self giveHardpoint( "radar_mp", streak );
	else if ( streak == (5 + offset) )
		self giveHardpoint( "airstrike_mp", streak );
	else if ( streak == (7 + offset) )
		self giveHardpoint( "helicopter_mp", streak );
	/*
	else if ( streak == (9 + offset) )
		self giveHardpoint( "predator_mp", streak );
	else if ( streak == (11 + offset) )
		self giveHardpoint( "pavelow_mp", streak );
	*/
	else if ( streak >= (21 + offset) )
	{
		if ( (streak % 5) == 0 )
			self streakNotify( streak );
	}
...
hardpointNotify( hardpointType, streakVal )
{
	...
	notifyData.titleLabel = &"MP_KILLSTREAK_N";
	notifyData.titleText = streakVal;
	if(hardpointType == "predator_mp" || hardpointType == "pavelow_mp")
		notifyData.notifyText = streakName(hardpointType);
	else
		...
		switch( currentWeapon )
		{
			...
			case "helicopter_mp":
				...
				{	
					...
					if(self.pers["hardPointItem"] != "predator_mp" && self.pers["hardPointItem"] != "pavelow_mp")
...
{
	if(hardpointType == "radar_mp" && self.pers["hardPointItem"] == "pavelow_mp")
	{
		if(isSet(level.pavelow["use"]))
		{
		...
		{
			self streakWarning("PAVELOW_INCOMING","FRIENDLY_PAVELOW","ENEMY_PAVELOW");
			self thread dioda\pavelow::Call_pavelow();
			return;
		}
	}
	else if(hardpointType == "radar_mp" && self.pers["hardPointItem"] == "predator_mp")
	{
		...
		else 
		{
			...
			self thread dioda\_predator::predator();
		}
	}
